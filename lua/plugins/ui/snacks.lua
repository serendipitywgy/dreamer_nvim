return {
  ...
  config = function()
    ...
    keymap("n", "<leader>fB", function() Snacks.picker.buffers({ hidden = true, nofile = true }) end, { desc = "Buffers (all)" })
    ...
    keymap({ "n", "x" }, "<leader>sW", pick("grep_word", { root = false }), { desc = "Visual selection or word (cwd)" })
    ...
    if package.loaded["todo-comments"] then
      ...
      keymap("n", "<leader>sT", function() Snacks.picker.todo_comments({ keywords = { "TODO", "FIX", "FIXME" } }) end, { desc = "Todo/Fix/Fixme" })
    end
